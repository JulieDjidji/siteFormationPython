...
Voici un schéma usuel qui donne un premier apreçu des différentes méthodes de *Machine Learning* : 
...
<p align="center">
<a href="http://www.7wdata.be/wp-content/uploads/2017/04/CheatSheet.png">
<img src="/images/ChoixMethodesMachineLearning.jpg" alt="ChoixMethodesMachineLearning" width="800" height="400"/>
</a>
</p>

On peut aussi s'appuyer sur le [*cheatsheet* de *scikit-learn*](http://1.bp.blogspot.com/-ME24ePzpzIM/UQLWTwurfXI/AAAAAAAAANw/W3EETIroA80/s1600/drop_shadows_background.png). Enfin, afin de comprendre le fonctionnement intuitif de chaque méthode, il peut être utile de consulter un cours de *Machine Learning*. 

Le site [*Machine Learning Mastery*](https://machinelearningmastery.com/machine-learning-algorithms-mini-course/) propose des explications intuitives sur les principales méthodes de *Machine Learning*. Le livre [*Introduction to Machine Learning with Python*](https://perso.limsi.fr/annlor/enseignement/ensiie/Intro_ML_Python.pdf) fournit quelques explications sur de nombreux algorithmes et précise le code python nécessaire pour les mettre en oeuvre.

## I) Commencer par prédire une variable catégorielle binaire : un problème de classification 

Pour réaliser ce tutoriel, on a enrichi les données précédentes avec des données socio-démographiques. On met donc à disposition une base, nommée *tableModelisation*, contenant les villes de plus de 20 000 habitants. Une indicatrice, nommée *plus50*, constituée à partir de la présence de l'absence de la ville dans la table de l'*isf* manipulée précédemment, indique si plus de 50 redevables paient l'*isf* dans la ville. Vous pouvez consulter le  dictionnaire des données s'appelle *Dictionnaire_tableModelisation*.
...
```python
tableMod[np.any(tableMod[['plus50','P15_NSCOL15P_DIPLMIN', 'P15_NSCOL15P_CAPBEP', 'P15_NSCOL15P_SUP',
...
scatter_matrix=scatter_matrix(z, alpha=0.9, figsize=(9, 9), diagonal='hist')
...
<ins>**Exercice 4**</ins> : Les trois variables issues de la table sur l'*isf* (le nombre de redevables, le patrimoine moyen et l'impôt moyen) sont manquantes pour les villes où ce nombre est inférieur à 50. Si ces valeurs manquantes étaient aléatoires, comment auriez-vouspu les imputer ?
...
*Scikit-learn* offre plusieurs stratégies d'imputation présentées globalement sur leur site sous ce [lien](https://scikit-learn.org/stable/modules/impute.html) ou sur ce [lien](https://scikit-learn.org/stable/modules/generated/sklearn.impute.SimpleImputer.html) qui présente les options de la fonction *SimpleImputer*.
...
D'autres méthodes de normalisation des données sont implémentées dans *scikit-learn*, vous pouvez les consulter sur leur [site](https://scikit-learn.org/stable/modules/preprocessing.html).
</div>
...
N'hésitez pas à consulter l'aide détaillée de [*scikit-learn*](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html) relative à la régression logitique.
...
plt.yticks(np.arange(top_features), feature_names[top_coefficients], rotation=0, ha='right')
plt.show()
```
...
La précision et le rappel (\textit{recall} en anglais) constituent deux critères qui mettent en exergue les capacités d'un classifieur. Ils sont notamment pertinents dans le cas où les classes sont déséquilibrées.
...
On peut utiliser la fonction fournissant la matrice de confusion brute qui est détaillée dans ce [lien](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.confusion_matrix.html). Afin d'obtenir un résultat plus visuel, on peut reprendre la [fonction](https://scikit-learn.org/stable/auto_examples/model_selection/plot_confusion_matrix.html#sphx-glr-auto-examples-model-selection-plot-confusion-matrix-py) proposée sur le site de *scikit-learn* pour mettre en forme cette matrice. Le code de cette fonction est présentée ci-dessous :
...
Vous pourriez réappliquer le code de la partie précédente en vous référant à la documentation *scikit-learn* sur la [régression linéaire](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LinearRegression.html).

Il est aussi possible d'inclure les différentes étapes jusqu'à la prédiction au sein d'une seule chaîne de traitement, dite *pipeline*. Pour la mettre en place, vous pouvez consulter la documentation sur les [pipelines](https://scikit-learn.org/stable/modules/generated/sklearn.pipeline.Pipeline.html) dans *scikit-learn*.  
